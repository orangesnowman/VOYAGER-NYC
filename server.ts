  ...
  wss.on("connection", async (clientWs, request) => {
    ...
    async function connectSession(modelName: string): Promise<any> {
      ...
      const languageInstruction = lang === "ES" 
        ? "\n\nLANGUAGE CONFIGURATION: The user's preferred language is Spanish (ES). You must speak strictly in Spanish as your default, main conversational language. Do NOT translate your own Spanish conversational dialogue, responses, or sentences into English. Keep the dialogue entirely in Spanish. Only use English when correcting the user's grammar, teaching specific English vocabulary words (e.g. day lessons), or when the user explicitly asks for a translation. IMPORTANT: Whenever the conversation language switches (e.g. from Spanish to English), you MUST output the exact tag '[SWITCH_LANG: EN]' in your text transcription. If you switch from English to Spanish, you MUST output '[SWITCH_LANG: ES]'. Do not say these tags out loud, just output them in your text transcription at the start of your turn."
